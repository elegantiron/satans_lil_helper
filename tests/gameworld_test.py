...
class TestGameWorld:
    ...
    def test_player_position(self, iteration: int) -> None:  # pylint: disable=unused-argument
        gameworld = GameWorld()
    ...
    def test_entity_spawn(
        ...
        y: int = 0
        while not chosen:
            x = gameworld.rng.randint(
                0, len(gameworld.map.tiles) - 1
            )
            y = gameworld.rng.randint(
                0, len(gameworld.map.tiles[x]) - 1
            )
