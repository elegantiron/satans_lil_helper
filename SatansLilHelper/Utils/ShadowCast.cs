...
public static class ShadowCast
{
    ...
    public static void ComputeVisibility(ICellGrid grid, Point gridPosn, float viewRadius)
    {
        ...
        grid.SetLight(new Point(gridPosn.X, gridPosn.Y), 0.0f);
    ...
    public static List<Point> GetArea(ICellGrid grid, Point gridPosn, float viewRadius)
    {
        List<Point> points = [gridPosn];
        for (int txidx = 0; txidx < s_octantTransform.Length; txidx++)
        {
    ...
    private static void HashCircle(
        ICellGrid grid,
        Point gridPosn,
        float viewRadius,
        int startColumn,
